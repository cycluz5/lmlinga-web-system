...
class DeathRequestAdminTest extends TestCase
{
    ...
    public function test_authorized_admin_can_list_death_requests(): void
    {
        ...
        $this->assertStringContainsString('Mortality', $html);
        $this->assertStringContainsString('Mortality - LMLinga', $html);
        $this->assertStringNotContainsString('Death Requests', $html);
    ...
    public function test_unauthorized_user_cannot_access_death_requests(): void
    {
        ...
        $this->get(route('death-requests.index'))
            ->assertForbidden();

        $this->actingAsStaff(StaffRole::BHW);
        $this->get(route('death-requests.show', $id))
            ->assertForbidden();

        $this->actingAsStaff(StaffRole::BNS);
        $this->post(route('death-requests.approve', $id))
            ->assertForbidden();
    ...
    public function test_admin_can_review_pending_request(): void
    {
        ...
        $this->actingAsStaff(StaffRole::ADMIN);
        $response = $this->get(route('death-requests.show', $request));
        ...
        $this->assertStringContainsString('aria-label="Back to Mortality"', $html);
        $this->assertDoesNotMatchRegularExpression(
        ...
        $this->assertStringContainsString('07/12/2026', $html);
    ...
    public function test_rejection_requires_reason_and_leaves_resident_active(): void
    {
        ...
        $this->actingAsStaff(StaffRole::BHW);
        $bhw = $this->get(route('health-records.death.show', [
                'householdNo' => 'HH-151',
                'memberId' => 'MB-002',
            ]));
        $bhw->assertOk();
