...
class HealthRecordsMaternalTest extends TestCase
{
    ...
    public function test_resident_maternal_page_renders(): void
    {
        ...
        $this->assertStringContainsString('data-mc-delivered-month', $html);
        $this->assertStringNotContainsString('High Risk Pregnancies', $html);
        $this->assertStringNotContainsString('Due for Prenatal Visit', $html);
        $this->assertStringNotContainsString('Incomplete Prenatal', $html);
        $this->assertStringNotContainsString('data-hr-mc-add', $html);
        ...
        $this->assertStringContainsString('data-hr-mc-month', $html);
        $this->assertStringContainsString('>Name</th>', $html);
        $this->assertStringContainsString('>Age</th>', $html);
        $this->assertStringContainsString('Gravida / Parity', $html);
        $this->assertStringContainsString('>Action</th>', $html);
        $this->assertStringNotContainsString('Prenatal Visits', $html);
    ...
    public function test_empty_resident_listing_shows_empty_state_without_demo_names(): void
    {
        ...
        $this->assertStringContainsString('No maternal care records are available.', $html);
        $this->assertStringNotContainsString('Rosa Lim', $html);
        ...
        $this->assertStringNotContainsString('data-hr-mc-row', $html);
    }

    public function test_listing_shows_persisted_maternal_pregnancy_record(): void
    {
        ...
        MaternalPregnancy::factory()->create([
            ...
            'edd' => '2026-08-08',
            'prenatal' => [
                'visit_1' => ['date' => '2026-01-10'],
            ],
        ]);

        $rows = HealthRecordsMaternal::rows();
        $this->assertCount(1, $rows);
        $this->assertSame('Maternal Resident', $rows[0]['full_name']);
        $this->assertSame('C', $rows[0]['age_group']);
        $this->assertSame('1-0', $rows[0]['gravida_parity']);

        $this->actingAsStaff(StaffRole::BHW);
        $response = $this->get(route('health-records.maternal.index'));
        $html = $response->getContent();

        $this->assertStringContainsString('Maternal Resident', $html);
    ...
    public function test_resident_and_non_resident_populations_remain_separated(): void
    {
        ...
        $this->actingAsStaff(StaffRole::BHW);
        $residentHtml = $this->get(route('health-records.maternal.index'))
            ->getContent();
        $this->actingAsStaff(StaffRole::BHW);
        $nonResidentHtml = $this->get(route('health-records.maternal.non-residents.index'))
            ->getContent();
    ...
    public function test_resident_listing_rows_have_complete_clinical_values_when_records_exist(): void
    {
        $household = Household::factory()->create(['zone' => 'Zone 4']);
        $resident = Resident::factory()->create([
            'household_id' => $household->id,
            'first_name' => 'Complete',
            'last_name' => 'Clinical',
        ...
        MaternalPregnancy::factory()->create([
            ...
            'edd' => '2026-09-07',
            'prenatal' => [
                'visit_1' => ['date' => '2026-02-05'],
                'visit_2' => ['date' => '2026-03-05'],
            ],
            'delivery' => ['delivery_type' => 'VD'],
        ]);
        ...
        foreach (HealthRecordsMaternal::rows() as $row) {
            foreach ([
                ...
                $this->assertNotSame('', $value, $row['full_name'].' missing '.$field);
            }
        }
    }
    ...
    public function test_non_resident_listing_rows_have_complete_clinical_values(): void
    {
        ...
        $this->actingAsStaff(StaffRole::BHW);
        $html = $this->get(route('health-records.maternal.non-residents.index'))
            ->getContent();
    ...
    {
        $this->actingAsStaff(StaffRole::BHW);
        $resident = $this->get(route('health-records.maternal.index'));
        $resident->assertOk();
        ...
        $this->actingAsStaff(StaffRole::BHW);
        $nonResident = $this->get(route('health-records.maternal.non-residents.index'));
    ...
    {
        $this->actingAsStaff(StaffRole::BNS);
        $response = $this->get(route('health-records.maternal.non-residents.create'));
    ...
    public function test_server_derives_bmi_and_ignores_submitted_bmi(): void
    {
        ...
        $this->actingAsStaff(StaffRole::BHW);
        $response = $this->from(route('health-records.maternal.non-residents.create'))
    ...
    public function test_cancel_does_not_create_a_non_resident_maternal_client(): void
    {
        ...
        $this->actingAsStaff(StaffRole::BHW);
        $this->get(route('health-records.maternal.non-residents.create'))
            ->assertOk();
    ...
    public function test_resident_listing_has_view_action_for_persisted_clients(): void
    {
        $household = Household::factory()->create(['zone' => 'Zone 1']);
        $resident = Resident::factory()->create([
            'household_id' => $household->id,
        ...
        $rows = HealthRecordsMaternal::rows();
        $this->assertCount(1, $rows);
        $viewUrl = (string) ($rows[0]['view_url'] ?? '');
        $this->assertNotSame('', $viewUrl);

        $this->actingAsStaff(StaffRole::BHW);
        $html = $this->get(route('health-records.maternal.index'))
            ->getContent();

        $this->assertStringContainsString('>Action</th>', $html);
        $this->assertStringContainsString('data-hr-mc-view', $html);
        $this->assertStringContainsString($viewUrl, $html);
    }

    public function test_summary_counts_only_include_deliveries_from_current_month(): void
    {
        $household = Household::factory()->create(['zone' => 'Zone 1']);
        $currentMonth = now()->format('Y-m');

        $deliveredResident = Resident::factory()->create([
            'household_id' => $household->id,
        ...
        MaternalPregnancy::factory()->create([
            'resident_id' => $deliveredResident->id,
            'status' => MaternalPregnancy::STATUS_COMPLETED,
            'registered_at' => $currentMonth.'-01',
            'lmp' => '2025-06-01',
            'gravida' => 1,
            'parity' => 0,
            'edd' => '2026-03-08',
            'delivery' => [
                'delivery_type' => 'VD',
                'datetime' => $currentMonth.'-15T10:00',
            ],
        ]);

        $olderResident = Resident::factory()->create([
            'household_id' => $household->id,
            'first_name' => 'Delivered',
            'last_name' => 'Older',
            'sex' => 'Female',
        ...
        MaternalPregnancy::factory()->create([
            ...
            'delivery' => [
                'delivery_type' => 'VD',
                'datetime' => '2024-02-10T10:00',
            ],
        ]);

        $summary = HealthRecordsMaternal::summaryCounts(HealthRecordsMaternal::rows());
        $this->assertSame(2, $summary['total']);
        $this->assertSame(1, $summary['delivered']);
        $this->assertSame(HealthRecordsMaternal::currentDeliveredMonthLabel(), $summary['delivered_month_label']);
    }
    ...
    public function test_non_resident_listing_has_view_action_for_each_eligible_client(): void
    {
        ...
        $residentListing = $this->get(route('health-records.maternal.index'));
        $residentListing->assertOk();
        $this->assertStringContainsString('>Action</th>', $residentListing->getContent());
    ...
    {
        $this->actingAsStaff(StaffRole::BNS);
        $ana = $this->get(route('health-records.maternal.non-residents.show', ['clientKey' => 'ana-p-villanueva']));
        $ana->assertOk();
        ...
        $this->assertStringContainsString('03/12/1998', $anaHtml);
        ...
        $this->assertMatchesRegularExpression(
            '/<dt>Date Birth:<\/dt>\s*<dd>\s*03\/12\/1998\s*<\/dd>/u',
            $anaHtml
        );
        $this->assertDoesNotMatchRegularExpression(
        ...
        $this->assertMatchesRegularExpression(
            '/data-hr-mc-nr-add-record[^>]*\bdisabled\b/u',
            $anaHtml
        );
        $this->assertStringContainsString('aria-disabled="true"', $anaHtml);
        $this->assertStringNotContainsString('lml-hr-mc-show__chevron', $anaHtml);
        $this->assertStringContainsString('Active pregnancy', $anaHtml);
        $this->assertStringNotContainsString('lml-hr-mc-show__status-meta', $anaHtml);
    ...
    {
        $this->actingAsStaff(StaffRole::BHW);
        $this->get(route('health-records.maternal.non-residents.show', ['clientKey' => 'ramon-t-bautista']))
            ->assertNotFound();
    ...
    public function test_pregnancy_summary_hides_delivery_type_for_active_pregnancy(): void
    {
        $ana = HealthRecordsNonResidentMaternal::findEligible('ana-p-villanueva');
        $this->assertNotNull($ana);
        $this->assertFalse((bool) $ana['is_delivered']);
        $this->assertSame('CS', $ana['delivery_type']);

        $summary = HealthRecordsNonResidentMaternal::pregnancySummary($ana);
        $this->assertNotNull($summary);
        $this->assertSame('Active pregnancy', $summary['status_label']);
        $this->assertSame('', $summary['delivery_type']);

        $delivered = HealthRecordsNonResidentMaternal::pregnancySummary(array_merge($ana, [
            'is_delivered' => true,
        ]));
        $this->assertSame('Delivered', $delivered['status_label']);
        $this->assertSame('CS', $delivered['delivery_type']);
    }

    public function test_missing_clinical_fields_are_not_fabricated(): void
    {
        $normalize = new \ReflectionMethod(HealthRecordsNonResidentMaternal::class, 'normalizeRecord');
        $normalize->setAccessible(true);
        $row = $normalize->invoke(null, [
            'key' => 'empty-clinical-fixture',
            'full_name' => 'Empty Clinical Fixture',
            'sex' => 'Female',
        ]);

        $this->assertSame('', $row['lmp']);
        $this->assertSame('', $row['gravida_parity']);
        $this->assertSame('', $row['edd']);
        $this->assertSame('', $row['delivery_type']);
        $this->assertSame('', $row['trimester']);
        $this->assertSame('', $row['prenatal_visits']);
        $this->assertNotSame('06-15-25', $row['lmp']);
        $this->assertNotSame('1-0', $row['gravida_parity']);
        $this->assertNotSame('03-22-26', $row['edd']);
        $this->assertNotSame('VD', $row['delivery_type']);
        $this->assertNotSame('2nd', $row['trimester']);
        $this->assertNotSame('2', $row['prenatal_visits']);
    }
