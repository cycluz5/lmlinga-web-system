...
class HouseholdProfilingHouseholdMemberViewTest extends TestCase
{
    ...
    public function test_child_care_panel_does_not_list_deworming_as_a_sibling(): void
    {
        $infantHtml = $this->get(route('household-profiling.members.show', [
            'householdNo' => 'HH-151',
            'memberId' => 'MB-009',
        ]))->assertOk()->getContent();

        $this->assertSame(1, substr_count($infantHtml, 'Child Immunization'));
        $this->assertSame(1, substr_count($infantHtml, 'School-Based Immunization'));
        $this->assertSame(1, substr_count($infantHtml, 'Child Nutrition'));
        $this->assertSame(0, substr_count($infantHtml, '>Deworming</'));
        ...
        $this->assertSame(1, substr_count($adultHtml, 'Child Immunization'));
        $this->assertSame(1, substr_count($adultHtml, 'School-Based Immunization'));
        $this->assertSame(1, substr_count($adultHtml, 'Child Nutrition'));
        $this->assertSame(0, substr_count($adultHtml, '>Deworming</'));
        $this->assertSame(1, substr_count($adultHtml, 'id="lml-hh-mv-child-care-toggle"'));
        $this->assertMatchesRegularExpression(
    ...
    public function test_remaining_health_summary_records_remain_visible(): void
    {
        ...
        $response->assertDontSee('data-hh-member-family-planning', false);
        $response->assertDontSee('data-hh-member-maternal-care', false);
        $response->assertSee('Death', false);
        $response->assertSee('data-hh-member-risk-assessment', false);
        $response->assertSee('data-hh-member-adult-immunization', false);
        ...
        $immUrl = route('household-profiling.members.adult-immunization', [
            'householdNo' => 'HH-151',
            'memberId' => 'MB-001',
        ]);
        ...
        $response->assertDontSee('href="'.e($fpUrl).'"', false);
        $response->assertSee('href="'.e($immUrl).'"', false);
        $response->assertDontSee('href="'.e($mcUrl).'"', false);
    ...
    public function test_member_edit_and_delete_controls_remain_present(): void
    {
        ...
        $response->assertDontSee('data-hh-member-view-delete', false);
