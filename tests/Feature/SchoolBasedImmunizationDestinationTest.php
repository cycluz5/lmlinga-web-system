...
class SchoolBasedImmunizationDestinationTest extends TestCase
{
    ...
    public function test_preview_safe_save_markup_allows_blank_and_has_no_form_action(): void
    {
        $response = $this->get(route(
            'household-profiling.members.school-based-immunization',
            $this->memberParams()
        ...
        // Demo GET remains preview-safe (no form action). Store route exists for
        // DB-09 Phase 2 server contract; Phase 3 will wire persisted residents.
        $this->assertStringContainsString('data-persistence="db"', $html);
        ...
        $this->assertFalse(Route::has('household-profiling.members.school-based-immunization.update'));
    ...
    /**
     * Source-level guard: page script implements one-way DATE → checkbox sync only.
     * Manual checkbox alone must not invent a date.
     */
    public function test_page_script_implements_one_way_date_to_checkbox_synchronization(): void
    {
        ...
        $this->assertStringContainsString(
            'DATE EXISTS → corresponding Vaccines Type checkbox',
            $js
        );
        $this->assertStringContainsString('syncCheckboxFromDateInput', $js);
        $this->assertStringContainsString('bindDateCheckboxSync', $js);
        $this->assertStringContainsString('SLOT_TYPE_KEYS', $js);
        $this->assertStringContainsString('checkbox.checked =', $js);

        // Must not invent dates from checkbox changes.
        $this->assertStringNotContainsString('data-auto-date', $js);
        $this->assertStringNotContainsString('syncCheckboxFromDateInput(form, field);', $js);
        $this->assertDoesNotMatchRegularExpression(
            '/addEventListener\(\s*[\'"](?:change|input)[\'"].{0,160}type=["\']checkbox["\']/is',
            $js
        );
