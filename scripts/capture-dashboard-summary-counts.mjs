...
const outDir = path.join('docs', 'qa', 'screenshots', 'dashboard-map-caption-overlap-fix');
...
for (const shot of shots) {
  ...
  const metrics = await page.evaluate(() => {
    ...
    return {
      ...
      captionAttribution: (() => {
        const cap = document.querySelector('.lml-dash-map__caption');
        const attr = document.querySelector('.lml-dash-map .leaflet-control-attribution');
        if (!cap || !attr) return null;
        const cr = cap.getBoundingClientRect();
        const ar = attr.getBoundingClientRect();
        const overlap = !(cr.right < ar.left || cr.left > ar.right || cr.bottom < ar.top || cr.top > ar.bottom);
        return {
          captionBottom: Math.round(cr.bottom),
          attrTop: Math.round(ar.top),
          gap: Math.round(ar.top - cr.bottom),
          overlap,
        };
      })(),
      fixtureNote: (() => {
        const el = document.querySelector('.lml-dash-home__fixture-note');
        if (!el) return null;
        const cs = getComputedStyle(el);
...
if (shots.some((s) => s.w === 390)) {
  ...
  const clips = [
    ...
    { file: 'mobile-390-map-caption-fixed.png', sel: '.lml-dash-panel--map' },
  ];
