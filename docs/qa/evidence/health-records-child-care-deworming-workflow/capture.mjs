...
const pages = [
  { key: 'summary', path: '/health-records/child-care/deworming' },
  { key: 'show', path: '/health-records/child-care/deworming/kristine-b-reyes' },
  { key: 'create', path: '/health-records/child-care/deworming/kristine-b-reyes/create' },
];
  ...
  for (const vp of viewports) {
    ...
    const metrics = await page.evaluate(() => {
      ...
      return {
        ...
        hasAdd: Boolean(document.querySelector('[data-hr-dw-add], [data-hr-dw-add-record]')),
        hasExport: Boolean(document.querySelector('[data-hr-dw-export]')),
        title: document.querySelector('.lml-topbar__title, .lml-hr-cc-nr__child-name, .lml-hr-child-care__title')?.textContent?.trim() ?? null,
      };
    });
