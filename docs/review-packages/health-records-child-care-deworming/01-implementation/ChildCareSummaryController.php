...
class ChildCareSummaryController extends Controller
{
    ...
    public function dewormingShow(string $childKey): View
    {
        $child = HealthRecordsDeworming::findChild($childKey);

        return view('pages.health-records.child-care-deworming-show', [
            'active' => 'child-care',
            'pageTitle' => 'Child Care | Deworming',
            'pageSubtitle' => 'Deworming record for the selected child.',
            'child' => $child,
            'childKey' => $childKey,
            'records' => $child !== null
                ? HealthRecordsDeworming::recordsFor((string) $child['key'])
                : [],
        ]);
    }

    public function dewormingCreate(string $childKey): View
    {
        $child = HealthRecordsDeworming::findChild($childKey);
        ...
        return view('pages.health-records.child-care-deworming-create', [
            'active' => 'child-care',
            'pageTitle' => 'Child Care | Deworming',
            'pageSubtitle' => 'Add a Deworming record for the selected child.',
            'child' => $child,
            'childKey' => $childKey,
