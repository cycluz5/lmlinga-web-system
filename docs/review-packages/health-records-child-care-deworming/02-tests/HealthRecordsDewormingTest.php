...
class HealthRecordsDewormingTest extends TestCase
{
    ...
    public function test_add_deworming_record_form_renders_authoritative_fields(): void
    {
        $showUrl = route('health-records.child-care.deworming.show', [
        ...
        $this->assertStringContainsString('Add a Deworming record for the selected child.', $html);
        $this->assertStringContainsString('Jacob A. Magistrado', $html);
        $this->assertStringContainsString('Not yet school-aged', $html);
        $this->assertStringContainsString('id="lml-hr-dw-add-title"', $html);
        $this->assertMatchesRegularExpression(
            '/id="lml-hr-dw-add-title"[^>]*>[\s\S]*bi-journal-medical[\s\S]*>\s*Add Deworming Record\s*</u',
            $html
        );
        $this->assertStringContainsString('ROUND INFORMATION', $html);
        $this->assertStringContainsString('for="lml-hr-dw-year"', $html);
        $this->assertStringContainsString('for="lml-hr-dw-round"', $html);
        $this->assertStringContainsString('for="lml-hr-dw-se"', $html);
        $this->assertStringContainsString('for="lml-hr-dw-date"', $html);
        $this->assertStringContainsString('for="lml-hr-dw-remarks"', $html);
        $this->assertStringContainsString('>1</option>', $html);
        $this->assertStringContainsString('>2</option>', $html);
        $this->assertStringContainsString('NHTS', $html);
        $this->assertStringContainsString('Non-NHTS', $html);
