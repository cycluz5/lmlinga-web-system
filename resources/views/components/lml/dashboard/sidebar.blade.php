...
@php
    /*
     | Shared order for Admin, BHW, BNS, and BSPO:
     ...
     |
     | Admin-only destinations stay visible for workers but are disabled
     | (User Management, Requests including Household Requests and Death Requests).
     |
    ...
    /*
     | FR-26: Admin-only pending death-request count. Workers never query or
     | receive this value. Cached PWA shells may show a stale count until a
     | fresh online navigation.
     */
    $pendingDeathRequests = $normalizedRole === 'admin'
        ? \App\Support\PendingDeathRequestCount::forAdminNav()
        : 0;
    ...
    <div class="lml-sidebar__inner">
            ...
            <ul class="lml-sidebar__list list-unstyled mb-0">
                @foreach ($visibleItems as $item)
                    @php
                        ...
                        $isAdminOnlyDisabled = ($item['adminOnly'] ?? false)
                    ...
                    <li @class(['lml-sidebar__item', 'lml-sidebar__item--collapse' => $isCollapse])>
                        @if ($isCollapse)
                            ...
                            <div
                                @class([
                                    ...
                                    'lml-sidebar__link--disabled' => $isAdminOnlyDisabled,
                                ])
                            ...
                            >
                                @if ($hasParentHref)
                                    ...
                                    >
                                        ...
                                        @if (($item['key'] ?? '') === 'requests' && $pendingDeathRequests > 0)
                                            <span
                                                class="lml-sidebar__attention-dot"
                            ...
                            >
                                @if ($hasActiveChild)
                                    <x-lml.dashboard.sidebar-collapse-children
                                        :children="$children"
                                        :active="$active"
                                        :admin-only-disabled="$isAdminOnlyDisabled"
                                        :pending-death-requests="$pendingDeathRequests"
                                    />
                                @else
                                    {{--
                        ...
                        @else
                            @if ($isAdminOnlyDisabled)
                                <span
                                    class="lml-sidebar__link lml-sidebar__link--disabled lml-focus-ring"
                                    data-lml-admin-only
                                    aria-disabled="true"
                                    tabindex="0"
                                    role="button"
        ...
        <div class="lml-sidebar__footer">
            <form method="post" action="{{ route('logout') }}" class="d-inline">
                @csrf
                <button type="submit" class="lml-sidebar__logout lml-focus-ring border-0 bg-transparent p-0">
