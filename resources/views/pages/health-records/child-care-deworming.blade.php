{{--
    Health Records — Child Care → Deworming barangay-wide monitoring (read-only).

    Data: App\Support\DewormingMonitoringService — residents with current-year deworming_records.
    Search/zone/sex/status filters operate client-side on that recorded list.
--}}
...
@section('content')
    ...
    <div
        ...
        data-dw-data-mode="db"
        data-total="{{ $totalRows }}"
        data-export-url="{{ route('health-records.child-care.deworming.report-builder') }}"
    >
        <div class="lml-hr-child-care__panel">
            ...
            >
                <div class="lml-hr-child-care__search">
                    <label class="visually-hidden" for="lml-hr-dw-search">Name of Member</label>
            ...
            <p class="lml-hr-child-care__results visually-hidden" data-hr-dw-results aria-live="polite">
                Showing {{ $totalRows }} of {{ $totalRows }} members
            </p>
            ...
            <div class="lml-hr-child-care__table-card lml-hr-child-care__table-card--deworming">
                <div
                    ...
                    @if ($totalRows === 0) hidden @endif
                >
                    <table class="lml-hr-child-care__table lml-hr-child-care__table--deworming">
                        <caption class="visually-hidden">
                            Barangay-wide Deworming monitoring by member name, age,
                            July round date, and January round date from persisted records.
                        </caption>
                        <colgroup>
                ...
                >
                    ...
                    <p class="lml-hr-child-care__empty-title" data-hr-dw-empty-title>
                        {{ $totalRows > 0 ? 'No matching members' : 'No Deworming records found.' }}
                    </p>
                    <p class="lml-hr-child-care__empty-hint" data-hr-dw-empty-hint>
                        {{ $totalRows > 0 ? 'Try adjusting search or filters.' : 'Child Care Deworming administrations recorded this year will appear here.' }}
                    </p>
                </div>
            </div>
        </div>
