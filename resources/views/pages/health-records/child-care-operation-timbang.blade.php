{{--
    ...
    Data: App\Support\HealthRecordsOperationTimbang / OperationTimbangMonitoringService
    — Child Care–eligible residents with latest matching timbang_records for the session.
    Year/month session controls reload via query params. Zone/sex/status/name filters
    operate client-side on the server-rendered session rows. Export remains UI-phase toast.
--}}
...
@section('content')
    ...
    <div
        ...
        data-ot-data-mode="database"
        data-ot-page-url="{{ $operationTimbangUrl }}"
        data-total="{{ $totalRows }}"
        data-selected-year="{{ $selectedYear }}"
        data-selected-month="{{ $selectedMonth }}"
        data-export-url="{{ route('health-records.child-care.operation-timbang.report-builder') }}"
    >
                        ...
                        >
                            @foreach ($monthSessions as $session)
                                @php
                                    ...
                                    $isFuture = ! empty($session['is_future']);
                                @endphp
                                <button
                                    type="button"
                                    class="lml-hr-ot-month-pill lml-focus-ring{{ $isSelected ? ' lml-hr-ot-month-pill--active' : '' }}{{ $isFuture ? ' lml-hr-ot-month-pill--future' : '' }}"
                                    role="tab"
                                    ...
                                    @if ($isFuture) data-ot-future="1" @endif
                                    aria-selected="{{ $isSelected ? 'true' : 'false' }}"
                                    @if ($isSelected) aria-current="true" @endif
                                    @if ($isFuture)
                                        disabled
                                        aria-disabled="true"
                                        title="Future weigh-in period is not available yet"
                                    @endif
                                >
                                    ...
                                    @endif
                                    @if ($isFuture)
                                        <span class="visually-hidden">(future period unavailable)</span>
                                    @endif
                                </button>
                            @endforeach
                        </div>
                ...
                >
                    <table class="lml-hr-child-care__table lml-hr-child-care__table--operation-timbang">
                        <caption class="visually-hidden">
                            ...
                            Rows are database-backed Child Care–eligible residents
                            for the selected monthly weigh-in session.
                        </caption>
                        <colgroup>
