...
@section('content')
    @php
        ...
        $visitedAtValue = old('visited_at', (string) ($visit['visited_at'] ?? ''));
        $remarksValue = old('remarks', (string) ($visit['remarks'] ?? ''));
        $idPrefix = $isEdit ? 'lml-hr-fp-nr-edit' : 'lml-hr-fp-nr-add';
        $formAction = $isEdit && $visitId
            ? route('health-records.family-planning.non-residents.visits.update', [
                'clientKey' => $clientKey,
                'visitId' => $visitId,
            ])
            : ($client !== null
                ? route('health-records.family-planning.non-residents.visits.store', [
    ...
    >
        ...
        @if ($client === null)
            <section class="lml-hr-fp-nr__not-found" aria-labelledby="lml-hr-fp-nr-nf-title">
                ...
                <p>No non-resident client matches this record.</p>
        ...
        @elseif ($isEdit && empty($visit))
            <section class="lml-hr-fp-nr__not-found" aria-labelledby="lml-hr-fp-nr-nf-title">
                ...
                <p>No visit matches this record for the selected client.</p>
