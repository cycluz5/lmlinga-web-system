...
@section('content')
    @php
        ...
        if ($isHouseholdProfilingDeworming) {
            ...
            $backLabel = 'Back to member health records';
        } else {
            $backUrl = isset($child['back_url'])
                ? $child['back_url']
                : route('health-records.child-care.deworming');
            $backLabel = isset($child['back_url'])
                ? 'Back to member health records'
                : 'Back to Deworming summary';
        }
        $createUrl = isset($child['create_url'])
            ? $child['create_url']
            : $backUrl;
        $records = $records ?? [];
        $canAddRecord = $canAddRecord ?? false;
    @endphp
    ...
    >
        ...
        <a
            href="{{ $backUrl }}"
            class="lml-hr-cc-nr__page-back lml-focus-ring{{ $isHouseholdProfilingDeworming ? ' lml-hr-cc-nr__page-back--labeled' : '' }}"
