...
function initDeathVerify(root) {
    ...
    const rejectForm = root.querySelector('[data-dr-reject] form');
    guardRejectDoubleSubmit(rejectForm);
}

export function guardRejectDoubleSubmit(form) {
    if (!form || form.dataset.drRejectGuard === 'true') {
        return;
    }

    form.dataset.drRejectGuard = 'true';
    form.addEventListener('submit', (event) => {
        const submit = form.querySelector('[data-dr-reject-submit]');
        if (submit?.disabled) {
            event.preventDefault();
            return;
        }
