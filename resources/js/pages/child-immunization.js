/**
 * Child Immunization destination — inline Immunization edit mode.
 *
 ...
 * preview summary values from sessionStorage after a preview Birth History return.
 *
 * Vaccines Type checkboxes are derived status-only (always disabled).
 * Edit mode enables dose date fields only — never type-status checkboxes.
 */
...
function getEditableFields(form) {
    // Exclude Vaccines Type status checkboxes (data-child-imm-type-status).
    return form.querySelectorAll('[data-child-imm-field]:not([data-child-imm-type-status])');
}
