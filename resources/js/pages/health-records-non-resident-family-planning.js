/**
 * Health Records → Family Planning → Non-Resident clients.
 */
...
function initNrListingDeleteDialog(root) {
    ...
    const deleteForm = root.querySelector('[data-hr-fp-nr-delete-form]');
    let pendingDeleteUrl = '';

    root.querySelectorAll('[data-hr-fp-nr-delete-client]').forEach((trigger) => {
        trigger.addEventListener('click', () => {
            pendingDeleteUrl = trigger.getAttribute('data-delete-url') || '';
    ...
    confirmBtn?.addEventListener('click', () => {
        if (deleteForm instanceof HTMLFormElement && pendingDeleteUrl !== '') {
        ...
        pendingDeleteUrl = '';
        closeNrDeleteDialog(root);
        showNrToast(root, 'Delete is not available.');
    });
...
function initNrListing(root) {
    ...
    exportBtn?.addEventListener('click', () => {
        ...
        if (!exportUrl) {
            showNrToast(root, 'Export is not available.');
            return;
        }
