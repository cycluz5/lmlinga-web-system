...
/**
 * Real PDF 1.4 Health Records → Risk Assessment export (no Blade / no
 * browser print). Same grouped-column, straight-row architecture as
 * MaternalCarePdf/HouseholdProfilingMemberPdf: a two-level table header (a
 * section group bar spanning its fields, then the individual field labels
 * below), one row per risk assessment record. Zone starts on a new page,
 * records sorted alphabetically by name. A row is atomic: it never splits
 * mid-row across a page break, it moves to the next page whole, behind a
 * small continuation strip identifying the zone and the name the table
 * resumes at. Column groups mirror HealthRecordsRiskAssessment::sections()
 * (Client, Red Flag Assessment, Past Medical History, Family History,
 * Lifestyle & Risk Factor, Physical Measurement and Clinical Screening) —
 * every field is always present, blank when not recorded.
 */
final class RiskAssessmentPdf
{
    ...
    private const BANNER_TEXT = 'HEALTH RECORDS — RISK ASSESSMENT RECORDS';
     ...
     */
    public static function response(array $rows, string $periodLabel, DateTimeInterface $generatedAt): Response
    {
        $filename = 'risk-assessment-'.$generatedAt->format('Ymd-His').'.pdf';

        return response(self::render($rows, $periodLabel, $generatedAt), 200, [
    ...
    /**
     * @param  list<array<string, mixed>>  $rows  Rows as produced by
     *                                             HealthRecordsRiskAssessment::detailedExportRows()
     *                                             — every Red Flag
     *                                             Assessment / Past
     ...
     */
    public static function render(array $rows, string $periodLabel, DateTimeInterface $generatedAt): string
    {
        ...
        $footer = $office.' · '.$periodLabel.' · Date of export: '.$exportDate;
        $logo = self::logoImage();
        ...
        // The full field column set never fits one legible page width, so
        // it's split into fixed topic pages (Client, Red Flag Assessment,
        // Past Medical History, Family History, Lifestyle & Risk Factor,
        // Physical Measurement and Clinical Screening) — Name repeats as
        // the identifying anchor column from page 2 onward — rather than
        // squeezing every column down until it's unreadable.
        ...
        $drawPageHeader = function () use (
            ...
            $commands = array_merge($commands, self::pageHeaderCommands($totalCount, $periodLabel, $hasLogo, $hasBold));
            $y = self::PAGE_HEIGHT - self::MARGIN_X - $pageHeaderH;
        };

        $drawZoneTitle = function (string $zoneName, string $groupTitle, int $partNumber, int $partCount) use (&$commands, &$y, $hasBold): void {
            $suffix = ' · '.$groupTitle.($partCount > 1 ? ' (PART '.$partNumber.' OF '.$partCount.')' : '');
            $label = strtoupper($zoneName).' REPORT'.$suffix;
        ...
        if ($rows === []) {
            $drawPageHeader();
            $commands[] = self::text(self::BODY_SIZE, self::MARGIN_X, $y, 'No risk assessment records found.');
            $y -= 28;
        ...
        $isFirstPage = true;
        foreach ($columnPages as $columnPage) {
            $sections = $columnPage['sections'];
            $fields = self::flattenFields($sections);
            $widths = self::columnWidths($fields);
            $tableHeaderH = self::groupedTableHeaderHeight($widths, $fields);

            foreach ($byZone as $zoneName => $zoneRows) {
                // Every zone (on every column page) begins on a fresh page.
                if (! $isFirstPage) {
                    $flush();
                }
                $isFirstPage = false;
                ...
                $drawPageHeader();
                $drawZoneTitle((string) $zoneName, $columnPage['groupTitle'], $columnPage['part'], $columnPage['partsInGroup']);
                $commands = array_merge($commands, self::drawGroupedTableHeader($sections, $fields, $widths, self::MARGIN_X, $y, $tableHeaderH, $hasBold));
                $y -= $tableHeaderH;
                ...
                foreach ($zoneRows as $row) {
                    ...
                    $rowH = self::measureRowHeight($fields, $widths, $row);
                    ...
                    $commands = array_merge($commands, self::drawTableRow($fields, $widths, $row, self::MARGIN_X, $y, $rowH, $rowIndex));
                    $y -= $rowH;
                    $rowIndex++;
                }
            }
    ...
    /**
     * @param  list<array{key: string, title: string, fields: list<array{key: string, label: string, width: float}>}>  $sections
     ...
     */
    private static function groupedTableHeaderHeight(array $widths, array $fields): float
    {
        return self::TABLE_GROUP_HEADER_H + self::tableHeaderHeight($widths, $fields);
    }

    /**
     * Two-level header: a section group bar spanning its own fields, then
     * the individual field labels below — same technique as
     * HouseholdProfilingMemberPdf's grouped columns.
     *
     * @param  list<array{key: string, title: string, fields: list<array{key: string, label: string, width: float}>}>  $sections
     * @param  list<array{key: string, label: string, width: float}>  $fields
     * @param  list<float>  $widths
     * @return list<string>
     */
    private static function drawGroupedTableHeader(array $sections, array $fields, array $widths, float $x, float $y, float $headerH, bool $hasBold): array
    {
        $commands = [];
        ...
        $commands = array_merge($commands, self::drawTableHeader($fields, $widths, $x, $fieldY, $fieldRowH, $hasBold));
    ...
    private static function rowsGroupedByZone(array $rows): array
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($zone === '' || $zone === HealthRecordsRiskAssessment::EMPTY) {
     ...
     * @return list<float>
     */
    private static function columnWidths(array $columns): array
    {
        return array_map(static fn (array $column): float => (float) $column['width'], $columns);
    }
     ...
     * @param  list<array{key: string, label: string, width: float}>  $columns
     */
    private static function tableHeaderHeight(array $widths, array $columns): float
    {
        $maxLines = 1;
     ...
     * @return list<string>
     */
    private static function drawTableHeader(array $columns, array $widths, float $x, float $y, float $headerH, bool $hasBold): array
    {
        $commands = [];
        $cx = $x;
     ...
     */
    private static function measureRowHeight(array $columns, array $widths, array $row): float
    {
        $maxLines = 1;
        foreach ($columns as $i => $column) {
     ...
     * @return list<string>
     */
    private static function drawTableRow(array $columns, array $widths, array $row, float $x, float $y, float $rowH, int $rowIndex): array
    {
        $commands = [];
        if ($rowIndex % 2 === 1) {
    ...
    private static function pageHeaderCommands(int $totalCount, string $periodLabel, bool $withLogo, bool $hasBold): array
    {
        ...
        self::drawInlineStat($commands, $statsX, $statsY, 'Overall Assessed Clients', $totalCount, $hasBold);

        $commands[] = self::text(self::META_SIZE, $statsX, $statsY - 14, $periodLabel, $font, 0.30, 0.30, 0.30);
