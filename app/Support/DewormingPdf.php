...
/**
 * Real PDF 1.4 Health Records → Child Care → Deworming export (no Blade /
 * no browser print). Same architecture as DeathRecordsPdf/ChildCarePdf:
 * legal landscape, Poppins body, zone starts on a new page, one table per
 * zone, one row per child, sorted alphabetically by name. A row is
...
final class DewormingPdf
{
    ...
    private const BANNER_TEXT = 'HEALTH RECORDS — DEWORMING MONITORING';
    ...
    private const COLUMNS = [
        ...
        ['key' => 'age_label', 'label' => 'Age', 'weight' => 0.8],
        ['key' => 'july_round', 'label' => 'July Round (Date)', 'weight' => 1.0],
        ['key' => 'january_round', 'label' => 'January Round (Date)', 'weight' => 1.0],
    ];
    ...
    {
        $filename = 'deworming-'.$generatedAt->format('Ymd-His').'.pdf';
    ...
    public static function render(array $rows, string $scopeLabel, DateTimeInterface $generatedAt): string
    {
        ...
        if ($rows === []) {
            $drawPageHeader();
            $commands[] = self::text(self::BODY_SIZE, self::MARGIN_X, $y, 'No deworming records found.');
            $y -= 28;
    ...
    private static function pageHeaderCommands(int $totalCount, string $scopeLabel, bool $withLogo, bool $hasBold): array
    {
        ...
        self::drawInlineStat($commands, $statsX, $statsY, 'Overall Deworming Population', $totalCount, $hasBold);
