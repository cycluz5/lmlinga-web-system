...
/**
 * Real PDF 1.4 Health Records → Child Care → Vitamin A export (no Blade /
 * no browser print). Mirrors the on-screen monitoring table exactly: the
 * same 3-tier grouped header (Age Group / Target / "Total Number of
 * Children Given Vitamin A" spanning the two dose groups, each split into
 * Male/Female/Total) and the same rows (the age bands plus a bolded
 * Total row) as HealthRecordsVitaminA::monitoringRows(). This is a single
 * small aggregate table, not a per-resident listing, so there is no zone
 * pagination or row-atomic page-break handling — the report is one page.
 */
final class VitaminAPdf
{
    ...
    private const BANNER_TEXT = 'HEALTH RECORDS — VITAMIN A MONITORING';
    ...
    /** Height of each of the three stacked header tiers. */
    private const HEADER_TIER_H = 20.0;

    /**
     * Fixed pixel widths matching the on-screen table's left-to-right
     * column order: Age Group, Target, [100k IU: Male/Female/Total],
     * [200k IU: Male/Female/Total], Percentage.
     *
     * @var list<array{key: string, label: string, width: float}>
     */
    private const COLUMNS = [
        ['key' => 'label', 'label' => "Age Group", 'width' => 150.0],
        ['key' => 'target', 'label' => "Target\n6mos to 6yrs old", 'width' => 100.0],
        ['key' => 'va_100k_male', 'label' => 'Male', 'width' => 90.0],
        ['key' => 'va_100k_female', 'label' => 'Female', 'width' => 90.0],
        ['key' => 'va_100k_total', 'label' => 'Total', 'width' => 90.0],
        ['key' => 'va_200k_male', 'label' => 'Male', 'width' => 90.0],
        ['key' => 'va_200k_female', 'label' => 'Female', 'width' => 90.0],
        ['key' => 'va_200k_total', 'label' => 'Total', 'width' => 90.0],
        ['key' => 'percentage', 'label' => "Percentage\nAccomplishment (%)", 'width' => 110.0],
    ];
    ...
    /**
     * @param  list<array<string, mixed>>  $rows  As produced by HealthRecordsVitaminA::monitoringRows().
     */
    public static function response(array $rows, string $scopeLabel, DateTimeInterface $generatedAt): Response
    {
        $filename = 'vitamin-a-'.$generatedAt->format('Ymd-His').'.pdf';
    ...
    public static function render(array $rows, string $scopeLabel, DateTimeInterface $generatedAt): string
    {
        ...
        $widths = array_map(static fn (array $c): float => (float) $c['width'], self::COLUMNS);
        $hasLogo = $logo !== null;
        $hasBold = $fontBold !== null;
        $targetTotal = self::targetTotal($rows);

        $commands = self::pageHeaderCommands($scopeLabel, $targetTotal, $hasLogo, $hasBold);
        $y = self::PAGE_HEIGHT - self::MARGIN_X - self::pageHeaderHeight($hasLogo);

        $commands = array_merge($commands, self::drawVitaminAHeader($widths, self::MARGIN_X, $y, $hasBold));
        $y -= self::HEADER_TIER_H * 3;

        $rowIndex = 0;
        foreach ($rows as $row) {
            $isTotal = ! empty($row['is_total']);
            $rowH = self::measureRowHeight($widths, $row);
            $commands = array_merge($commands, self::drawTableRow($widths, $row, self::MARGIN_X, $y, $rowH, $rowIndex, $isTotal, $hasBold));
            $y -= $rowH;
            $rowIndex++;
        }
     ...
     * @param  list<array<string, mixed>>  $rows
     */
    private static function targetTotal(array $rows): int
    {
        foreach ($rows as $row) {
            if (! empty($row['is_total'])) {
                $value = trim((string) ($row['target'] ?? ''));
    ...
    /**
     * Draws the 3-tier grouped header: Age Group / Target / Percentage as
     * tall standalone columns spanning all three tiers, and the two dose
     * groups (100,000 IU / 200,000 IU) as a colspan-6 umbrella label over
     * their own Male/Female/Total sub-columns — exactly the structure of
     * the on-screen table's <thead>.
     *
     * @param  list<float>  $widths
     * @return list<string>
     */
    private static function drawVitaminAHeader(array $widths, float $x, float $y, bool $hasBold): array
    {
        $commands = [];
        $font = $hasBold ? 'F2' : 'F1';
        $headerH = self::HEADER_TIER_H * 3;
        ...
        foreach ([0, 1, 8] as $i) {
            $commands[] = self::filledBar($offsets[$i], $y - $headerH, $widths[$i], $headerH, 0.24, 0.65, 0.33);
            $commands[] = self::rect($offsets[$i], $y - $headerH, $widths[$i], $headerH);
            $commands = array_merge($commands, self::drawWrappedCenteredText(
                (string) self::COLUMNS[$i]['label'],
                $offsets[$i],
                $y - ($headerH / 2),
                $widths[$i],
                $font
        ...
        $groupX = $offsets[2];
        $groupW = array_sum(array_slice($widths, 2, 6));
        $commands[] = self::filledBar($groupX, $y - self::HEADER_TIER_H, $groupW, self::HEADER_TIER_H, 0.16, 0.50, 0.26);
        $commands[] = self::rect($groupX, $y - self::HEADER_TIER_H, $groupW, self::HEADER_TIER_H);
        $commands[] = self::centeredHeaderText('Total Number of Children Given Vitamin A', $groupX, $groupW, $y - self::HEADER_TIER_H + 6, $font);

        $tier2Y = $y - self::HEADER_TIER_H;
        foreach ([[2, '100,000'], [5, '200,000']] as [$startIdx, $doseLabel]) {
            $doseX = $offsets[$startIdx];
            $doseW = array_sum(array_slice($widths, $startIdx, 3));
            $commands[] = self::filledBar($doseX, $tier2Y - self::HEADER_TIER_H, $doseW, self::HEADER_TIER_H, 0.24, 0.65, 0.33);
            $commands[] = self::rect($doseX, $tier2Y - self::HEADER_TIER_H, $doseW, self::HEADER_TIER_H);
            $commands[] = self::centeredHeaderText('Vitamin A '.$doseLabel.' IU', $doseX, $doseW, $tier2Y - self::HEADER_TIER_H + 6, $font);
        }

        $tier3Y = $tier2Y - self::HEADER_TIER_H;
        for ($i = 2; $i <= 7; $i++) {
            $commands[] = self::filledBar($offsets[$i], $tier3Y - self::HEADER_TIER_H, $widths[$i], self::HEADER_TIER_H, 0.30, 0.72, 0.40);
            $commands[] = self::rect($offsets[$i], $tier3Y - self::HEADER_TIER_H, $widths[$i], self::HEADER_TIER_H);
            $commands[] = self::centeredHeaderText((string) self::COLUMNS[$i]['label'], $offsets[$i], $widths[$i], $tier3Y - self::HEADER_TIER_H + 6, $font);
        }

        return $commands;
    }

    private static function centeredHeaderText(string $text, float $x, float $w, float $y, string $font): string
    {
        $textW = self::approxTextWidth($text, self::TABLE_HEADER_SIZE);
        $tx = $x + max(0.0, ($w - $textW) / 2);

        return self::text(self::TABLE_HEADER_SIZE, $tx, $y, $text, $font, 1, 1, 1);
    }

    /**
     * @return list<string>
     */
    private static function drawWrappedCenteredText(string $label, float $x, float $centerY, float $w, string $font): array
    {
        $commands = [];
        $lines = explode("\n", $label);
        $lineH = self::TABLE_HEADER_SIZE + self::TABLE_LINE_GAP;
        $totalH = count($lines) * $lineH;
        $startY = $centerY + ($totalH / 2) - (self::TABLE_HEADER_SIZE * 0.8);

        foreach ($lines as $i => $line) {
            $textW = self::approxTextWidth($line, self::TABLE_HEADER_SIZE);
            $tx = $x + max(0.0, ($w - $textW) / 2);
            $commands[] = self::text(self::TABLE_HEADER_SIZE, $tx, $startY - ($i * $lineH), $line, $font, 1, 1, 1);
        }
    ...
    private static function measureRowHeight(array $widths, array $row): float
    {
        ...
        foreach (self::COLUMNS as $i => $column) {
            $value = self::fieldValue($row, (string) $column['key']);
     ...
     * @return list<string>
     */
    private static function drawTableRow(array $widths, array $row, float $x, float $y, float $rowH, int $rowIndex, bool $isTotal, bool $hasBold): array
    {
        $commands = [];
        if ($isTotal) {
            $commands[] = self::filledBar($x, $y - $rowH, array_sum($widths), $rowH, 0.90, 0.96, 0.91);
        } elseif ($rowIndex % 2 === 1) {
            $commands[] = self::filledBar($x, $y - $rowH, array_sum($widths), $rowH, 0.97, 0.98, 0.97);
        }

        $font = $isTotal && $hasBold ? 'F2' : 'F1';
        $cx = $x;
        foreach (self::COLUMNS as $i => $column) {
            $w = $widths[$i];
            $value = self::fieldValue($row, (string) $column['key']);
    ...
    private static function pageHeaderHeight(bool $withLogo): float
    {
        return $withLogo ? 122.0 : 110.0;
    }

    private static function pageHeaderCommands(string $scopeLabel, int $targetTotal, bool $withLogo, bool $hasBold): array
    {
        ...
        self::drawInlineStat($commands, $statsX, $statsY, 'Overall Target Population', $targetTotal, $hasBold);
