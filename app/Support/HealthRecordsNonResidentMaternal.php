...
final class HealthRecordsNonResidentMaternal
{
    ...
    public static function pregnancySummary(array $client): ?array
    {
        ...
        $deliveryType = trim((string) ($client['delivery_type'] ?? ''));
        ...
        return [
            ...
            'delivery_type' => $delivered ? $deliveryType : '',
    ...
    public static function formatDisplayDate(string $value): string
    {
        ...
        try {
            if (preg_match('/^\d{2}-\d{2}-\d{2}$/', $trimmed) === 1) {
                $parsed = Carbon::createFromFormat('m-d-y', $trimmed);
                $formatted = DisplayDate::format($parsed?->toDateString());

                return $formatted !== '' ? $formatted : $trimmed;
            }

            $formatted = DisplayDate::format($trimmed);
    ...
    private static function normalizeRecord(array $row): array
    {
        ...
        return [
            ...
            'lmp' => self::storedClinicalValue((string) ($row['lmp'] ?? '')),
            'gravida_parity' => self::storedClinicalValue((string) ($row['gravida_parity'] ?? '')),
            'edd' => self::storedClinicalValue((string) ($row['edd'] ?? '')),
            'weight' => $row['weight'] ?? null,
            'height' => $row['height'] ?? null,
            'bmi' => $row['bmi'] ?? null,
            'blood_pressure' => (string) ($row['blood_pressure'] ?? ''),
            'delivery_type' => self::storedClinicalValue((string) ($row['delivery_type'] ?? '')),
            'trimester' => self::storedClinicalValue((string) ($row['trimester'] ?? '')),
            'prenatal_visits' => self::storedClinicalValue((string) ($row['prenatal_visits'] ?? '')),
    ...
    private static function storedClinicalValue(string $value): string
    {
        $trimmed = trim($value);
