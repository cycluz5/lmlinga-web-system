...
/**
 * Real PDF 1.4 Health Records → Maternal export (no Blade / no browser
 * print). Same grouped-column, straight-row architecture as
 * HouseholdProfilingMemberPdf: a two-level table header (a section group
 * bar spanning its fields, then the individual field labels below), one
 * row per maternal care record. Zone starts on a new page, records sorted
 * alphabetically by name. A row is atomic: it never splits mid-row across
 * a page break, it moves to the next page whole, behind a small
 * continuation strip identifying the zone and the name the table resumes
 * at. Column groups mirror HealthRecordsMaternal::sections(), which in
 * turn mirrors the Household Profiling member Maternal Care history
 * form's own section order (Registration, Prenatal Visits,
 * Supplementation, Laboratory Screening, Pregnancy Delivery & Outcome,
 * Postnatal Care) — every field is always present, "—" when not recorded.
 */
final class MaternalCarePdf
{
    ...
    private const BANNER_TEXT = 'HEALTH RECORDS — MATERNAL CARE RECORDS';
    ...
    {
        $filename = 'maternal-care-'.$generatedAt->format('Ymd-His').'.pdf';
     ...
     *                                             HealthRecordsMaternal::detailedExportRows()
     *                                             — every Registration /
     *                                             Prenatal Visits /
     *                                             Supplementation /
     *                                             Laboratory Screening /
     *                                             Delivery & Outcome /
     *                                             Postnatal Care field is
    ...
    public static function render(array $rows, string $periodLabel, DateTimeInterface $generatedAt): string
    {
        ...
        // The full ~100-field column set never fits one legible page width,
        // so it's split into six fixed topic pages (Client & Registration,
        // Prenatal Visits, Supplementation, Laboratory Screening, Pregnancy
        // Delivery & Outcome, Postnatal Care) — Name repeats as the
        // identifying anchor column from page 2 onward — rather than
        ...
        if ($rows === []) {
            $drawPageHeader();
            $commands[] = self::text(self::BODY_SIZE, self::MARGIN_X, $y, 'No maternal care records found.');
            $y -= 28;
    ...
    private static function rowsGroupedByZone(array $rows): array
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($zone === '' || $zone === HealthRecordsMaternal::EMPTY) {
    ...
    private static function pageHeaderCommands(int $totalCount, string $periodLabel, bool $withLogo, bool $hasBold): array
    {
        ...
        self::drawInlineStat($commands, $statsX, $statsY, 'Overall Maternal Count', $totalCount, $hasBold);
