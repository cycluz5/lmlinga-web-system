...
/**
 * Real PDF 1.4 Health Records → Child Care → Operation Timbang export (no
 * Blade / no browser print). Same architecture as
 * DeathRecordsPdf/ChildCarePdf/DewormingPdf: legal landscape, Poppins
 * body, zone starts on a new page, one table per zone, one row per
 * child, sorted alphabetically by name. A row is atomic: it never splits
 ...
 * resumes at.
 */
final class OperationTimbangPdf
{
    ...
    private const BANNER_TEXT = 'HEALTH RECORDS — OPERATION TIMBANG MONITORING';
    ...
    private const SUMMARY_CARD_GAP = 12.0;

    private const SUMMARY_CARD_H = 68.0;

    private const SUMMARY_LABEL_SIZE = 8;

    private const SUMMARY_VALUE_SIZE = 16;

    /**
     * @var list<array{key: string, label: string, weight: float}>
     */
    private const COLUMNS = [
        ['key' => 'full_name', 'label' => 'Name', 'weight' => 1.5],
        ['key' => 'age_label', 'label' => 'Age', 'weight' => 0.7],
        ['key' => 'weight', 'label' => 'Weight', 'weight' => 0.6],
        ['key' => 'height', 'label' => 'Height', 'weight' => 0.6],
        ['key' => 'muac', 'label' => 'MUAC', 'weight' => 0.6],
        ['key' => 'status_label', 'label' => 'Status', 'weight' => 0.7],
    ];
     ...
     * @param  array<string, mixed>  $summary  As produced by HealthRecordsOperationTimbang::summaryCards() — the
     *                                         on-screen "Summary: {session}" panel, rendered as its own page
     *                                         ahead of the per-zone data tables. Omitted (empty array) skips it.
     */
    public static function response(
        ...
        $filename = 'operation-timbang-'.$generatedAt->format('Ymd-His').'.pdf';

        return response(self::render($rows, $scopeLabel, $generatedAt, $summary, $sessionLabel), 200, [
    ...
    public static function render(
        ...
        if ($summary !== []) {
            $drawPageHeader();
            $summaryTitle = 'SUMMARY: '.strtoupper($sessionLabel !== '' ? $sessionLabel : $scopeLabel);
            $commands[] = self::text(self::TITLE_SIZE, self::MARGIN_X, $y, $summaryTitle, $hasBold ? 'F2' : 'F1');
            $y -= 8;
            $commands[] = self::filledBar(self::MARGIN_X, $y, $contentWidth, 1.4, 0.24, 0.65, 0.33);
            $y -= 24;
            $commands = array_merge($commands, self::drawSummaryCards($summary, self::MARGIN_X, $y, $contentWidth, $hasBold));
            $flush();
        }

        if ($rows === []) {
            $drawPageHeader();
            $commands[] = self::text(self::BODY_SIZE, self::MARGIN_X, $y, 'No Operation Timbang records found.');
            $y -= 28;
    ...
    /**
     * Mirrors the on-screen "Summary: {session}" panel: 7 single-value
     * cards in a 4-column grid, plus one wider Male/Female split card.
     *
     * @param  array<string, mixed>  $summary
     * @return list<string>
     */
    private static function drawSummaryCards(array $summary, float $x, float $y, float $contentWidth, bool $hasBold): array
    {
        $cards = [
            ['label' => 'No. of 0-23 Months PS', 'value' => (string) ($summary['ps_0_23'] ?? '0')],
            ['label' => 'No. of 0-23 Months Old Measured', 'value' => (string) ($summary['measured_0_23'] ?? '0')],
            ['label' => 'No. of Over age', 'value' => (string) ($summary['over_age'] ?? '0')],
            ['label' => 'No. of Transferred/Moveout', 'value' => (string) ($summary['transferred'] ?? '0')],
            ['label' => 'No. of Dead', 'value' => (string) ($summary['dead'] ?? '0')],
            ['label' => 'No. Not Available', 'value' => (string) ($summary['not_available'] ?? '0')],
            ['label' => 'No. of New Cases', 'value' => (string) ($summary['new_cases'] ?? '0')],
            [
                'label' => 'Total Number of 0-23 Months',
                'value' => 'M – '.(string) ($summary['total_male'] ?? '0').'   F – '.(string) ($summary['total_female'] ?? '0'),
            ],
        ];

        $commands = [];
        $cols = 4;
        $gap = self::SUMMARY_CARD_GAP;
        $cardW = ($contentWidth - ($gap * ($cols - 1))) / $cols;
        ...
        foreach ($cards as $i => $card) {
            ...
            $commands[] = self::rect($cx, $cy - $cardH, $cardW, $cardH);
            $commands = array_merge($commands, self::wrapLinesCommands(
                $card['label'],
                $cx + 10,
                $cy - 16,
                $cardW - 20,
                self::SUMMARY_LABEL_SIZE,
                0.42,
                0.45,
                0.49
            ));
            $commands[] = self::text(
                self::SUMMARY_VALUE_SIZE,
                $cx + 10,
                $cy - $cardH + 22,
                $card['value'],
                $hasBold ? 'F2' : 'F1',
     ...
     * @return list<string>
     */
    private static function wrapLinesCommands(
        string $text,
        float $x,
        float $topY,
        float $maxWidth,
        ...
        $commands = [];
        $step = $fontSize + 2;
        foreach (self::wrap($text, $maxWidth, $fontSize) as $i => $line) {
            $commands[] = self::text($fontSize, $x, $topY - ($i * $step), $line, 'F1', $r, $g, $b);
        }
    ...
    private static function pageHeaderCommands(int $totalCount, string $scopeLabel, bool $withLogo, bool $hasBold): array
    {
        ...
        self::drawInlineStat($commands, $statsX, $statsY, 'Overall Weigh-In Population', $totalCount, $hasBold);
