...
/**
 * Real PDF 1.4 Health Records → Family Planning export (no Blade / no
 * browser print). Same architecture as DeathRecordsPdf: legal landscape,
 * Poppins body, zone starts on a new page, one table per zone, one row
 * per commodity given (a single visit with several commodities produces
 * several rows), sorted alphabetically by name then visit date. A row is
 * atomic: it never splits mid-row across a page break, it moves to the
 * next page whole, behind a small continuation strip identifying the
 * zone and the name the table resumes at.
 */
final class FamilyPlanningPdf
{
    ...
    private const BANNER_TEXT = 'HEALTH RECORDS — FAMILY PLANNING COMMODITIES';
    ...
    private const COLUMNS = [
        ...
        ['key' => 'visit_date', 'label' => 'Visit Date', 'weight' => 0.9],
        ['key' => 'commodity', 'label' => 'Commodity', 'weight' => 1.1],
        ['key' => 'quantity', 'label' => 'Quantity', 'weight' => 0.6],
    ];
    ...
    {
        $filename = 'family-planning-'.$generatedAt->format('Ymd-His').'.pdf';
    ...
    public static function render(array $rows, string $periodLabel, DateTimeInterface $generatedAt): string
    {
        ...
        if ($rows === []) {
            $drawPageHeader();
            $commands[] = self::text(self::BODY_SIZE, self::MARGIN_X, $y, 'No family planning commodity records found.');
            $y -= 28;
     ...
     * records are sorted alphabetically by name (then visit date).
     *
     * @param  list<array<string, mixed>>  $rows
    ...
    private static function rowsGroupedByZone(array $rows): array
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($zone === '' || $zone === HealthRecordsFamilyPlanning::EMPTY) {
    ...
    private static function pageHeaderCommands(int $totalCount, string $periodLabel, bool $withLogo, bool $hasBold): array
    {
        ...
        self::drawInlineStat($commands, $statsX, $statsY, 'Overall Commodities Given', $totalCount, $hasBold);
