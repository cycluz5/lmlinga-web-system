...
final class DeathCertificateStorage
{
    ...
    public static function store(UploadedFile $file, DeathRequest $request): array
    {
        ...
        if (DeathRecordsErdMode::isActive()) {
            return [
                'death_certificate_file_path' => $path,
            ];
        }
    ...
    public static function deleteStored(?DeathRequest $request): void
    {
        ...
        $disk = DeathRecordsErdMode::isActive()
            ? self::DISK
            : (string) $request->certificate_disk;
    ...
    public static function download(DeathRequest $request): StreamedResponse
    {
        ...
        $downloadName = DeathRecordsErdMode::isActive()
            ? DemoDeath::safeFilename(basename($path)) ?: 'death-certificate.pdf'
            : (DemoDeath::safeFilename((string) $request->certificate_original_name)
        ...
        return Storage::disk($disk)->response(
            ...
            $downloadName,
            [
                'Content-Type' => DeathRecordsErdMode::isActive()
                    ? (DemoDeath::mimeForPublicExtension(pathinfo($path, PATHINFO_EXTENSION)) ?? 'application/octet-stream')
                    : (string) ($request->certificate_mime ?: 'application/octet-stream'),
