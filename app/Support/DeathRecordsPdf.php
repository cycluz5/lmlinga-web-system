...
/**
 * Real PDF 1.4 Health Records → Death export (no Blade / no browser
 * print). Same architecture as EnvironmentalHealthPdf /
 * HouseholdProfilingMemberPdf: legal landscape, Poppins body, zone starts
 * on a new page, one table per zone, one row per (admin-verified) death
 * record, sorted alphabetically by name. A row is atomic: it never splits
...
final class DeathRecordsPdf
{
    ...
    private const BANNER_TEXT = 'HEALTH RECORDS — VERIFIED DEATH RECORDS';
    ...
    private const COLUMNS = [
        ...
        ['key' => 'age', 'label' => 'Age', 'weight' => 0.5],
        ['key' => 'birthday', 'label' => 'Birthday', 'weight' => 0.9],
        ['key' => 'sex', 'label' => 'Sex', 'weight' => 0.6],
        ['key' => 'cause_of_death', 'label' => 'Cause of Death', 'weight' => 2.2],
    ];
    ...
    public static function render(array $rows, string $periodLabel, DateTimeInterface $generatedAt): string
    {
        ...
        if ($rows === []) {
            $drawPageHeader();
            $commands[] = self::text(self::BODY_SIZE, self::MARGIN_X, $y, 'No verified death records found.');
            $y -= 28;
    ...
    private static function rowsGroupedByZone(array $rows): array
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($zone === '' || $zone === HealthRecordsDeath::EMPTY) {
    ...
    private static function pageHeaderCommands(int $totalCount, string $periodLabel, bool $withLogo, bool $hasBold): array
    {
        ...
        self::drawInlineStat($commands, $statsX, $statsY, 'Overall Death Count', $totalCount, $hasBold);
